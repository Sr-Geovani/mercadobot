...
async def gerar_link_pagamento(asaas_cliente_id: str, chat_id: int, reativacao: bool = False,
    ...
    proxima_cobranca_em: se fornecido (formato ISO), usa como nextDueDate
    em vez de calcular automaticamente. Útil para agendamentos em 
    cancelado_mas_ativo.
    """
    if proxima_cobranca_em:
        # Usa a data customizada passada (ex: data de assinatura_fim)
        primeiro_vencimento = proxima_cobranca_em
        descricao = "MercadoBot — Renovação agendada de assinatura"
    ...
    async with httpx.AsyncClient() as client:
        ...
        data = resp.json()
        logger.info(f"[CHECKOUT] Criado: {data.get('id')}")

        checkout_id = data.get("id", "")
        link = data.get("link", "")

        if not link:
            logger.error(f"Erro: {data.get('errors')}")
            return "", ""
